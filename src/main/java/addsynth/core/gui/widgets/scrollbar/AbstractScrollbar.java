...
public abstract class AbstractScrollbar<E, L extends AbstractListEntry<E>> extends Widget {
  ...
  public AbstractScrollbar(int x, int y, int height, L[] list_items, E[] values){
    super(x, y, scrollbar_gui_width, height, new StringTextComponent(""));
  ...
  private final void recalculateScrollbar(){
    try{
      ...
      if(list_length > visible_elements){
        ...
        index_position = MathHelper.clamp(index_position, 0, max_positions - 1);
      }
      else{
  ...
  protected final void onDrag(double gui_x, double gui_y, double screen_x, double screen_y){
    ...
    position_y = MathHelper.clamp(center_y - scrollbar_half_height, this.y, max_position_y);
  ...
  public final void mouseScrollWheel(int direction){
    final int temp_index = MathHelper.clamp(index_position - direction, 0, max_positions - 1);
  ...
  private final void scroll_to_value(){ // seperate as a function in case we need to call it externally?
    if(hasValidSelection()){
      final int index = MathHelper.clamp(selected - (visible_elements / 2), 0, index_positions.length - 1);
