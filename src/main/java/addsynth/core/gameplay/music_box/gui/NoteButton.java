...
public final class NoteButton extends Widget {
  ...
  public NoteButton(int x_position, int y_position, byte track, byte frame, TileMusicBox tile){
    super(x_position, y_position , GuiMusicBox.note_button_width, GuiMusicBox.note_button_height, new StringTextComponent(note[tile.get_note(frame, track)]));
    this.tile = tile;
  ...
  public void render(MatrixStack matrix, int p_render_1_, int p_render_2_, float p_render_3_){
    ...
    if(visible){
      setMessage(new StringTextComponent(note[tile.get_note(frame, track)])); // OPTIMIZE: Find everywhere we're setting the widget message each tick, and don't creat new TextComponents, have them already defined as static somewhere.
    }
    super.render(matrix, p_render_1_, p_render_2_, p_render_3_);
  }

  @Override
  public final void renderButton(MatrixStack matrix, final int mouseX, final int mouseY, final float partial){
