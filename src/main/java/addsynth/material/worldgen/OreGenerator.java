...
public final class OreGenerator {
  ...
  public static final void register(Category category, BiomeGenerationSettingsBuilder generation_settings){
    // This is how it's done now apparently.
    // for(Biome biome : ForgeRegistries.BIOMES){ // OPTIMIZE: set a veriable for biome category
  ...
  private static final void generate_ore(BiomeGenerationSettings.Builder builder, OreMaterial material, int size, int tries, int min, int max){
    if(valid_min_max_values(material.name, min, max)){
      ...
      */
      final OreFeatureConfig ore_feature = new OreFeatureConfig(OreFeatureConfig.FillerBlockType.NATURAL_STONE, material.ore.defaultBlockState(), size);
      builder.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Feature.ORE.configured(ore_feature).range(max_depth).squared().count(tries));
      /*
