      ...
      case X:
        ...
        for(y = start_y; y < start_y + 3; y++){
          for(x = start_x; x < start_x + 3; x++){
            level.setBlockAndUpdate(new BlockPos(x,y,z),Portal.portal.defaultBlockState()); // .withProperty(PortalEnergyBlock.AXIS, EnumFacing.Axis.X));
          }
        }
        if(Features.unknown_dimension.get() == false){
        }
        WorldUtil.spawnItemStack(level, center, start_y, start_z, new ItemStack(Init.void_crystal, 1), false);
        break;
      case Z:
        ...
        for(y = start_y; y < start_y + 3; y++){
          for(z = start_z; z < start_z + 3; z++){
            level.setBlockAndUpdate(new BlockPos(x,y,z),Portal.portal.defaultBlockState()); // .withProperty(PortalEnergyBlock.AXIS, EnumFacing.Axis.Z));
          }
        }
        if(Features.unknown_dimension.get() == false){
        }
        WorldUtil.spawnItemStack(level, start_x, start_y, center, new ItemStack(Init.void_crystal, 1), false);
        break;
      }
