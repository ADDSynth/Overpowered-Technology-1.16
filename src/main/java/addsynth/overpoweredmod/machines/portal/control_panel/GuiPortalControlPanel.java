...
public final class GuiPortalControlPanel extends GuiEnergyBase<TilePortalControlPanel, ContainerPortalControlPanel> {
  ...
  public final void init(){
    super.init();
    addButton(new OnOffSwitch<>(this.leftPos + 6, this.topPos + 19, tile));
    addButton(new AutoShutoffCheckbox<TilePortalControlPanel>(this.leftPos + checkbox_x, this.topPos + checkbox_y, tile));
    addButton(new GeneratePortalButton(this.leftPos + button_x, this.topPos + button_y, tile));
   ...
   */
  private final void draw_portal_items(final MatrixStack matrix){
    ...
    minecraft.getTextureManager().bind(gui_icons);
    for(j = 0; j < 2; j++){
      for(i = 0; i < 4; i++){
        index = (j*4) + i;
  ...
  @Override
  protected void renderTooltip(MatrixStack matrix, int mouse_x, int mouse_y){
